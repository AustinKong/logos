...
Do not add `from __future__ import annotations` by default. Use it only when a file actually needs postponed annotation evaluation, such as avoiding string annotations for forward references. ORM model files are the explicit exception described in the database model guidance below.
...
The command runs Alembic autogenerate, then Ruff formats and fix-checks migration files. Review autogenerated migrations after the command runs.
...
The TUI dev command runs the app with Textual dev mode and opens the Textual console in a second terminal when a supported terminal launcher is available. If the console cannot be opened automatically, run `npm run dev:tui:console` in another terminal.
